...
## Let's add a new dimension to this analysis: Change over time

# We will add a year column so we can look at voting trends over time
by_country_year <- un_votes_joined %>%
  ...
  ungroup() %>% # It is recommended to ungroup before filter steps. Grouping adds metadata that can affect later transformations, and ungrouping can make later steps more efficient.
  filter(number >= 10) # filter by records with more than 10 observations

library(broom) # Provides cleaner output for our linear model coefficients

# For US
us <- by_country_year %>%
  filter(country == "United States of America")

lm(percent_yes ~ year, us) %>% summary() %>% tidy()
# Shows that percent yes decreases by 0.6% each year

# For China
cn <- by_country_year %>%
  filter(country == "China")

lm(percent_yes ~ year, cn) %>% summary() %>% tidy()
# Shows that percent yes decreases by 0.02% each year, but p value is not high enough to determine a strong relationship.

# Using previous code, let's look at the plots of the US and China
countries <- c("United States of America", "China")
