...
```{r}
### Load data from unvotes to local csv files
library(unvotes)
library(readr)
...
# Tokenization involves breaking up strings into pieces (e.g. words, keywords, phrases). This is a common task in text mining.

un_roll_calls

un_roll_call_words <- un_roll_calls %>%
  select(rcid, short) %>%
  filter(!is.na(short)) %>% # is.na() is an r function that identifies null values
  unnest_tokens(word, short) %>% # unnest the 'short' column by words
  anti_join(stop_words %>% filter(word != "self"), by = "word") # To remove stop words

# Plot the most common words used
un_roll_call_words %>%
